...
export class ToolRepository {
  ...
  async createTool(input: CreateToolInput): Promise<ToolDefinition> {
    ...
    if (!input.embeddingModel || input.embeddingDimension <= 0) {
      throw new ValidationError(
        'Valid embeddingModel and positive embeddingDimension are required',
      );
    }

    if (input.embedding) {
      this.vectorStore.validateEmbedding(
        input.embedding,
  ...
  async getToolByName(name: string): Promise<ToolDefinition | null> {
    ...
    const db = await this.provider.getDbAsync();
    try {
      const result = await db.execute(`SELECT * FROM tools WHERE name = ?`, [
        name,
      ]);
      const rows = result.rows ?? [];
      if (rows.length === 0) return null;
  ...
  async updateTool(
    ...
    const newDimension =
      input.embeddingDimension ?? existing.embeddingDimension;
    const newModel = input.embeddingModel ?? existing.embeddingModel;

    if (
      (newDimension !== existing.embeddingDimension ||
        newModel !== existing.embeddingModel) &&
      !input.embedding
    ) {
      throw new ValidationError(
