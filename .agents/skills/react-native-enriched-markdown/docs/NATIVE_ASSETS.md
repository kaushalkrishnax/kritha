...
**Which `package.json`?** The _download_ opt-out is read from wherever the install runs (via `INIT_CWD`)
— in a monorepo that is the workspace root, and there is one shared `node_modules`, so the download
opt-out is global. The _build_ flag is read from the **app's** `package.json` (the one beside `ios/` /
