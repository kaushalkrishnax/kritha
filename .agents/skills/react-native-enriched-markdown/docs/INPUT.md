...
- **List items are single-line.** Each item is exactly one paragraph. Importing or pasting Markdown with _loose_ list items (an item containing multiple paragraphs) keeps only the item's first line as a list item — continuation paragraphs become regular paragraphs, not list-styled blocks.
...
```tsx
<EnrichedMarkdownTextInput linkRegex={/https?:\/\/[^\s]+/i} />
```
